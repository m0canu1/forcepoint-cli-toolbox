...
printf '%s\n' 'Checking that dist scripts do not depend on repository-local files...'
if grep -nE '(source|\.)[[:space:]]+.*(compat\.sh|scripts/|\.\./)' dist/*.sh 2>/dev/null; then
    echo 'A dist script appears to source another repository file.' >&2
    failed=1
fi
